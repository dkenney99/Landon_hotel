    ...
    onSubmit({value,valid}:{value:Roomsearch,valid:boolean}){
      this.getAll().subscribe(rooms => {
        ...
        this.rooms.forEach((room) => {
            // Assign conversion for Euro
            room.priceEURO = (Number(room.price) * 0.91).toFixed(2);
            // Assign conversion for Canadian dollar
            room.priceCAD = (Number(room.price) * 1.33).toFixed(2);
          })
        }
      );
    }
